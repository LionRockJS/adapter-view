...
export default class LiquidView extends View {
  static VIEW_PATH = "";
  static moduleSnippets: Set<string> = new Set();
  ...
  resolveView(file: string, default_file: string = ""): string {
    const fetchedView: any = Central.resolveView(file);
    if(!fetchedView) {
  ...
  private async ensureSettings() {
    if (this.data.settings !== undefined) return;
  ...
  getEngine(extraRoot: string[] = []){
    const root = new Set(
      [`${LiquidView.VIEW_PATH}/sections`, `${LiquidView.VIEW_PATH}/snippets`, ...LiquidView.moduleSnippets.values(), `${this.themePath}/sections`, `${this.themePath}/snippets`, ...extraRoot]
    );
  ...
  async liquidRender(){
    ...
    if(!this.resolvedView?.payload) throw new Error(`LiquidView: resolved view payload is undefined for view "${this.file}" (realPath: "${this.realPath}")`);
  ...
  static async parseSettings(engine: any, node: any, data: any){
    if(/{{.*}}|{%.*%}/.test(node.type)){
  ...
  async jsonRender(){
    ...
    for(let key of template.order){
      ...
      if(/^#/.test(section.type)){
        ...
        if(Array.isArray(section.settings.value)){
          section.settings.value.forEach((it: any) => metaEntry.add(it));
        }
      ...
      if(section.block_order && Array.isArray(section.block_order) && section.block_order.length > 0){
        section.blocks = section.block_order.map((it: any) => section.blocks[it]);

        //blocks selections
        await Promise.all(
          section.blocks.map(async (block: any) => LiquidView.parseSettings(engine, block, this.data))
        )
      }
    ...
    let result = template.order.map((it: string) => renders[it]).join('\n');
