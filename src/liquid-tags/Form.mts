export default {
  ...
  parse(token: any, remainTokens: any) {
    ...
    const stream = (this as any).liquid.parser.parseStream(remainTokens)
      .on('start', () => {
        (this as any).lead = `<form method="post" accept-charset="UTF-8" ${form} ${attributes.join(' ')}>\n${
      ...
      .on('tag:endform', (tpl: any) => {
        (this as any).trail = '</form>';
        stream.stop();
      })
      .on('end', () => {
